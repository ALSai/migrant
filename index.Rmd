...
```{r echo=FALSE}
years <- sort(unique(format(dataset$time, format="%Y")))
```

In this database, there are `r format(nrow(dataset), big.mark=",")` rows with :

* `r length(unique(dataset$citizen))` different origins,
* `r length(unique(dataset$geo))` different destinations,
* `r length(years)` different years (from `r head(years, n=1)` to `r tail(years, n=1)`).
